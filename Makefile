...
ifndef USE_CURL
  # The bundled Windows libcurl predates the UCRT toolchain used by current
  # MSYS2 releases. Keep clean Windows builds linkable by default; builders
  # with a current libcurl can opt in with USE_CURL=1.
  ifdef MINGW
    USE_CURL=0
  else
    USE_CURL=1
...
# NVIDIA Streamline/DLSS is an optional Windows x64 Vulkan feature.  The SDK
# itself is fetched separately; see docs/DLSS.md.
ifndef USE_NVIDIA_DLSS
  ifeq ($(PLATFORM),mingw64)
    ifeq ($(ARCH),x86_64)
      USE_NVIDIA_DLSS=1
    else
...
ifndef STREAMLINE_DIR
STREAMLINE_DIR=$(BUILD_DIR)/deps/streamline-sdk-v2.12.0
endif
...
STREAMLINE_INCLUDEDIR=$(STREAMLINE_DIR)/include
STREAMLINE_BINDIR=$(STREAMLINE_DIR)/bin/x64
NGX_INCLUDEDIR=$(STREAMLINE_DIR)/external/ngx-sdk/include
...
ifdef MINGW
  ...
  # Use generic windres if the cross-prefixed program is not present.
  ifeq ($(call bin_path, $(WINDRES)),)
    WINDRES=windres
  endif
  ...
  ifeq ($(USE_NVIDIA_DLSS),1)
    BASE_CFLAGS += -DUSE_NVIDIA_STREAMLINE -I$(STREAMLINE_INCLUDEDIR) -I$(NGX_INCLUDEDIR)
    STREAMLINE_LIBS += -lstdc++ -lwintrust -lcrypt32
  endif
...
# Windows applications do not need an architecture suffix in their filename.
# Keep the traditional suffix on other platforms where parallel architecture
# builds commonly share one output directory.
ifndef CLIENTBINEXT
  ifdef MINGW
    CLIENTBINEXT=$(BINEXT)
  else
    CLIENTBINEXT=$(FULLBINEXT)
  endif
endif
...
ifneq ($(BUILD_CLIENT),0)
  ifneq ($(USE_RENDERER_DLOPEN),0)
    TARGETS += $(B)/$(CLIENTBIN)$(CLIENTBINEXT) $(B)/renderer_opengl1_$(SHLIBNAME) 
...
##################  VULKAN  ###################
    ...
    ifeq ($(USE_NVIDIA_DLSS),1)
      ...
      TARGETS += $(STREAMLINE_RUNTIME_TARGETS) \
		$(B)/nvngx.dll $(B)/dlssnr-3rd-party-notice.txt \
        $(B)/streamline-license.txt $(B)/streamline-3rd-party-licenses.md \
        $(B)/nvngx_dlss.license.txt $(B)/reflex.license.txt
    endif
###############################################
  else
    TARGETS += $(B)/$(CLIENTBIN)$(CLIENTBINEXT)
    ifneq ($(BUILD_RENDERER_OPENGL2),0)
      TARGETS += $(B)/$(CLIENTBIN)_opengl2$(CLIENTBINEXT)
    endif
##################  VULKAN  ###################
    TARGETS += $(B)/$(CLIENTBIN)_vulkan$(CLIENTBINEXT)
###############################################
  endif
endif
...
ifneq ($(USE_RENDERER_DLOPEN),0)
$(B)/$(CLIENTBIN)$(CLIENTBINEXT): $(Q3OBJ) $(LIBSDLMAIN)
	$(echo_cmd) "LD $@"
...
$(B)/renderer_vulkan_$(SHLIBNAME): $(Q3VKOBJ) $(JPGOBJ)
	...
	$(Q)$(CC) $(CFLAGS) $(SHLIBLDFLAGS) -o $@ $(Q3VKOBJ) $(JPGOBJ) \
		$(THREAD_LIBS) $(RENDERER_LIBS) $(LIBSDLMAIN) $(LIBS) $(STREAMLINE_LIBS)

##############################################################


else
$(B)/$(CLIENTBIN)$(CLIENTBINEXT): $(Q3OBJ) $(Q3ROBJ) $(JPGOBJ) $(LIBSDLMAIN)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) $(NOTSHLIBLDFLAGS) \
		-o $@ $(Q3OBJ) $(Q3ROBJ) $(JPGOBJ) \
		$(LIBSDLMAIN) $(CLIENT_LIBS) $(RENDERER_LIBS) $(LIBS)

$(B)/$(CLIENTBIN)_opengl2$(CLIENTBINEXT): $(Q3OBJ) $(Q3R2OBJ) $(Q3R2STRINGOBJ) $(JPGOBJ) $(LIBSDLMAIN)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) $(NOTSHLIBLDFLAGS) \
...
######################## VULKAN ##############################
$(B)/$(CLIENTBIN)_vulkan$(CLIENTBINEXT): $(Q3OBJ) $(Q3VKOBJ) $(JPGOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) $(CLIENT_CFLAGS) $(CFLAGS) $(CLIENT_LDFLAGS) $(LDFLAGS) $(NOTSHLIBLDFLAGS) \
		-o $@ $(Q3OBJ) $(Q3VKOBJ) $(JPGOBJ) \
		$(LIBSDLMAIN) $(CLIENT_LIBS) $(RENDERER_LIBS) $(LIBS) $(STREAMLINE_LIBS)
...
$(B)/renderer_vulkan/%.o: $(RVULKANDIR)/%.cpp
	$(echo_cmd) "REF_CXX $<"
	$(Q)$(CXX) $(SHLIBCFLAGS) $(CFLAGS) $(CLIENT_CFLAGS) $(OPTIMIZE) -std=gnu++17 -o $@ -c $<

$(B)/renderer_vulkan/%.o: $(MOUNT_DIR)/renderer_vulkan/shaders/Compiled/%.c
	$(DO_REF_CC)

ifeq ($(USE_NVIDIA_DLSS),1)
$(B)/nvngx.dll: $(RVULKANDIR)/vk_ngx_bridge.cpp
	$(echo_cmd) "NVNGX_BRIDGE $<"
	$(Q)$(CXX) $(CFLAGS) $(OPTIMIZE) -std=gnu++17 -shared -static-libgcc -static-libstdc++ -o $@ $<

$(B)/%.dll: $(STREAMLINE_BINDIR)/%.dll
	$(echo_cmd) "COPY $@"
	$(Q)cp $< $@

$(B)/streamline-license.txt: $(STREAMLINE_DIR)/license.txt
	$(Q)cp $< $@

$(B)/streamline-3rd-party-licenses.md: $(STREAMLINE_DIR)/3rd-party-licenses.md
	$(Q)cp $< $@

$(B)/nvngx_dlss.license.txt: $(STREAMLINE_BINDIR)/nvngx_dlss.license.txt
	$(Q)cp $< $@

$(B)/reflex.license.txt: $(STREAMLINE_BINDIR)/reflex.license.txt
	$(Q)cp $< $@

$(B)/dlssnr-3rd-party-notice.txt: docs/DLSSNR-THIRD-PARTY-NOTICE.txt
	$(Q)cp $< $@
endif
...
ifneq ($(BUILD_CLIENT),0)
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/$(CLIENTBIN)$(CLIENTBINEXT) $(COPYBINDIR)/$(CLIENTBIN)$(CLIENTBINEXT)
  ifneq ($(USE_RENDERER_DLOPEN),0)
    ...
    ifneq ($(BUILD_RENDERER_OPENGL2),0)
	$(INSTALL) $(STRIP_FLAG) -m 0755 $(BR)/$(CLIENTBIN)_opengl2$(CLIENTBINEXT) $(COPYBINDIR)/$(CLIENTBIN)_opengl2$(CLIENTBINEXT)
    endif
  endif
endif
