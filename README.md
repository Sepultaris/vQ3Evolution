...
VQ3 Evolution is a modernized Quake III engine focused on Vulkan rendering,
widescreen support, scalable UI and HUD rendering, and NVIDIA neural rendering.
It is derived from vkQuake3 and ioquake3 and retains compatibility with Quake III
game data and the `baseq3` directory layout.

Some of the major features currently implemented are:

* SDL 2 backend
* Aspect-correct widescreen rendering with Hor+ field of view, scalable menus,
  and edge-aware HUD placement
...
## Widescreen and UI scaling

The 3D view expands horizontally on widescreen and ultrawide displays without
stretching. Centered HUD elements remain centered, while corner status items
use the physical screen edges.

The in-game HUD and menus can be scaled independently from the console:

```
/seta cg_hudScale 1.0
/seta ui_scale 1.0
```

Both settings accept values from `0.5` to `1.5` and update immediately. The
default is `1.0`. They are also available as 50%-150% sliders under
`Setup > System > Graphics`.

For local games, the engine continues loading these freshly built source
modules after pure-file checks activate. Remote pure servers still use only
the gameplay modules approved by that server.
...
5. Find `vQ3Evolution.exe` and its DLLs in build/release-mingw64-x86\_64.
...
```
  ...
  BUILD_CLIENT         - build the VQ3 Evolution client binary
  ...
  CLIENTBIN            - rename the VQ3 Evolution client binary
...
```
  ...
  r_ext_max_anisotropy              - anisotropic filtering level, 1 to 16;
                                      Vulkan clamps it to the GPU limit
