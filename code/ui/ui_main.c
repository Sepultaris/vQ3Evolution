...
static void UI_UpdateScreenScale( void );
...
static void UI_UpdateScreenScale( void ) {
	...
	baseScale = uiInfo.uiDC.glconfig.vidWidth / 640.0f;
	if ( uiInfo.uiDC.glconfig.vidHeight / 480.0f < baseScale ) {
		baseScale = uiInfo.uiDC.glconfig.vidHeight / 480.0f;
	}

	userScale = UI_ClampCvar( 0.5f, 1.5f, ui_scale.value );
	uiInfo.uiDC.xscale = uiInfo.uiDC.yscale = baseScale * userScale;
	uiInfo.uiDC.bias = 0.5f * ( uiInfo.uiDC.glconfig.vidWidth - 640.0f * uiInfo.uiDC.xscale );
	uiInfo.uiDC.ybias = 0.5f * ( uiInfo.uiDC.glconfig.vidHeight - 480.0f * uiInfo.uiDC.yscale );
}
...
void _UI_Init( qboolean inGameLoad ) {
  ...
  uiInfo.aliasCount = 0;
	...
	if (trap_Cvar_VariableValue("ui_TeamArenaFirstRun") == 0) {
		// Sound registration owns factory defaults. Do not replace saved audio
		// preferences (or the release preset) merely by opening Team Arena.
...
static cvarTable_t		cvarTable[] = {
	{ &ui_scale, "ui_scale", "1.0", CVAR_ARCHIVE },
	...
	{ &ui_hudScale, "cg_hudScale", "1.0", CVAR_ARCHIVE },
