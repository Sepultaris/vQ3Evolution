...
static void UI_UpdateScreenScale( void ) {
	...
	baseScale = uis.glconfig.vidWidth / 640.0f;
	if ( uis.glconfig.vidHeight / 480.0f < baseScale ) {
		baseScale = uis.glconfig.vidHeight / 480.0f;
	}

	userScale = UI_ClampCvar( 0.5f, 1.5f, ui_scale.value );
	uis.xscale = uis.yscale = baseScale * userScale;
	uis.bias = 0.5f * ( uis.glconfig.vidWidth - 640.0f * uis.xscale );
	uis.ybias = 0.5f * ( uis.glconfig.vidHeight - 480.0f * uis.yscale );
}
...
void UI_AdjustFrom640( float *x, float *y, float *w, float *h ) {
	if ( *x == 0 && *y == 0 && *w == 640 && *h == 480 ) {
		*x = 0;
		*y = 0;
		*w = uis.glconfig.vidWidth;
		*h = uis.glconfig.vidHeight;
		return;
	}
