...
static int CG_CalcFov( void ) {
	...
	/*
	 * cg_fov is the horizontal FOV of a 4:3 display.  Preserve its vertical
	 * FOV on wider displays and expand the horizontal FOV (Hor+) so that
	 * widescreen does not crop or zoom the scene.
	 */
	aspect = cgs.glconfig.windowAspect;
	if ( aspect <= 0.0f ) {
		aspect = (float)cg.refdef.width / (float)cg.refdef.height;
	}
	if ( aspect > ( 4.0f / 3.0f ) ) {
		fov_y = atan2( 3.0f, 4.0f / tan( fov_x / 360.0f * M_PI ) );
		fov_y = fov_y * 360.0f / M_PI;
		fov_x = atan2( aspect * tan( fov_y / 360.0f * M_PI ), 1.0f );
