...
static cvarTable_t cvarTable[] = {
	...
	{ &cg_hudScale, "cg_hudScale", "1.0", CVAR_ARCHIVE },
...
/*
========================
CG_UpdateScreenPlacement

Keeps the 640x480 HUD aspect-correct, then applies the user's HUD scale.
========================
*/
static void CG_UpdateScreenPlacement( void ) {
	float baseScale;
	float hudScale;

	baseScale = cgs.glconfig.vidWidth / 640.0f;
	if ( cgs.glconfig.vidHeight / 480.0f < baseScale ) {
		baseScale = cgs.glconfig.vidHeight / 480.0f;
	}

	hudScale = Com_Clamp( 0.5f, 1.5f, cg_hudScale.value );
	cgs.screenXScale = cgs.screenYScale = baseScale * hudScale;
	cgs.screenXBias = 0.5f * ( cgs.glconfig.vidWidth - 640.0f * cgs.screenXScale );
	cgs.screenYBias = 0.5f * ( cgs.glconfig.vidHeight - 480.0f * cgs.screenYScale );

#ifdef MISSIONPACK
	// The menu-driven Team Arena HUD shares the UI layout code, including
	// model previews that read the display transform directly.
	cgDC.xscale = cgs.screenXScale;
	cgDC.yscale = cgs.screenYScale;
	cgDC.bias = cgs.screenXBias;
	cgDC.ybias = cgs.screenYBias;
