...
vm_t *VM_Restart(vm_t *vm, qboolean unpure)
{
	...
	if ( vm->dllHandle ) {
		...
		vm = VM_Create( name, systemCall, VMI_NATIVE, unpure );
		return vm;
	}
...
vm_t *VM_Create( const char *module, intptr_t (*systemCalls)(intptr_t *), 
				vmInterpret_t interpret, qboolean allowUnpure ) {
	vm_t		*vm;
	vmHeader_t	*header;
	...
	do
	{
		retval = FS_FindVM(&startSearch, filename, sizeof(filename), module,
			(interpret == VMI_NATIVE), allowUnpure);
		...
		else if(retval == VMI_COMPILED)
		{
			...
			if((header = VM_LoadQVM(vm, qtrue, allowUnpure)))
				break;
