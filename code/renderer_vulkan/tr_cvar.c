...
cvar_t* r_postBloom;
cvar_t* r_postBloomStrength;
cvar_t* r_postBloomThreshold;
cvar_t* r_postBloomDebug;
cvar_t* r_nvNightVision;
cvar_t* r_nvTint;
cvar_t* r_nvGrain;
cvar_t* r_nvVignette;
cvar_t* r_nvBrightness;
cvar_t* r_nvOverride;
cvar_t* r_nvDebug;
...
cvar_t* r_muzzleFlashBrightness;
cvar_t* r_rocketBrightness;
cvar_t* r_rocketLightScale;
cvar_t* r_rocketExplosionLightScale;
cvar_t* r_lightningGunLightScale;
cvar_t* r_muzzleFlashLightScale;
cvar_t* r_pathTracingSunAngle;
cvar_t* r_pathTracingSunScale;
cvar_t* r_pathTracingLightRadius;
cvar_t* r_pathTracingAutoExposure;
cvar_t* r_pathTracingAdaptiveSpeed;
cvar_t* r_pathTracingAdaptiveTarget;
cvar_t* r_pathTracingAdaptiveMin;
cvar_t* r_pathTracingAdaptiveMax;
...
cvar_t* r_pathTracingDlightReservoir;
...
void R_Register( void ) 
{
	...
	r_postBloom = ri.Cvar_Get( "r_postBloom", "1", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_postBloom, 0, 1, qfalse );
	r_postBloomStrength = ri.Cvar_Get( "r_postBloomStrength", "0.6", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_postBloomStrength, 0.0f, 4.0f, qfalse );
	r_postBloomThreshold = ri.Cvar_Get( "r_postBloomThreshold", "0.5", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_postBloomThreshold, 0.0f, 1.0f, qfalse );
	r_postBloomDebug = ri.Cvar_Get( "r_postBloomDebug", "0", CVAR_TEMP );
	ri.Cvar_CheckRange( r_postBloomDebug, 0, 1, qfalse );
	r_nvNightVision = ri.Cvar_Get( "r_nvNightVision", "1", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_nvNightVision, 0, 1, qfalse );
	r_nvTint = ri.Cvar_Get( "r_nvTint", "1", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_nvTint, 0, 1, qfalse );
	r_nvGrain = ri.Cvar_Get( "r_nvGrain", "0.4", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_nvGrain, 0.0f, 1.0f, qfalse );
	r_nvVignette = ri.Cvar_Get( "r_nvVignette", "1.0", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_nvVignette, 0.0f, 1.0f, qfalse );
	r_nvBrightness = ri.Cvar_Get( "r_nvBrightness", "1.6", CVAR_ARCHIVE );
	ri.Cvar_CheckRange( r_nvBrightness, 0.0f, 8.0f, qfalse );
	r_nvOverride = ri.Cvar_Get( "r_nvOverride", "0", CVAR_TEMP );
	ri.Cvar_CheckRange( r_nvOverride, 0, 1, qfalse );
	...
	ri.Cvar_SetDescription( r_pathTracingScale,
		"Path tracing internal render scale. Values < 1 run the tracer at a fraction of the "
		"render resolution and upscale the result; fractions snap to the nearest NVIDIA DLSS "
		"preset render scale (0.667 Quality, 0.583 Balanced, 0.5 Performance, 0.25 Ultra "
		"Performance). Changing this value requires a video restart." );
	...
	r_muzzleFlashBrightness = ri.Cvar_Get("r_muzzleFlashBrightness", "1", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_muzzleFlashBrightness, 0.0f, 8.0f, qfalse);
	ri.Cvar_SetDescription(r_muzzleFlashBrightness, "Visible tagged muzzle-flash brightness: 0 off, 1 original, up to 8. Live; separate from emitted light.");
	r_muzzleFlashLightScale = ri.Cvar_Get("r_muzzleFlashLightScale", "1", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_muzzleFlashLightScale, 0.0f, 8.0f, qfalse);
	ri.Cvar_SetDescription(r_muzzleFlashLightScale, "Tagged muzzle-flash illumination: dynamic light plus path-traced surface emission. 0 off, 1 original, up to 8; live.");
	r_rocketBrightness = ri.Cvar_Get("r_rocketBrightness", "1", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_rocketBrightness, 0.0f, 8.0f, qfalse);
	ri.Cvar_SetDescription(r_rocketBrightness, "Visible flying-rocket exhaust/glow: 0 off, 1 original, up to 8; live. Does not darken the opaque body.");
	r_rocketLightScale = ri.Cvar_Get("r_rocketLightScale", "1", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_rocketLightScale, 0.0f, 8.0f, qfalse);
	ri.Cvar_SetDescription(r_rocketLightScale, "Flying-rocket illumination: moving dynamic light plus traced exhaust emission. 0 off, 1 original, up to 8; live. Not explosions.");
	r_rocketExplosionLightScale = ri.Cvar_Get("r_rocketExplosionLightScale", "1", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_rocketExplosionLightScale, 0.0f, 8.0f, qfalse);
	ri.Cvar_SetDescription(r_rocketExplosionLightScale, "Rocket impact illumination: dynamic light plus traced explosion/particle emission. 0 off, 1 original, up to 8; live. Visible effects unchanged.");
	r_lightningGunLightScale = ri.Cvar_Get("r_lightningGunLightScale", "1", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_lightningGunLightScale, 0.0f, 8.0f, qfalse);
	ri.Cvar_SetDescription(r_lightningGunLightScale, "Lightning-gun illumination: traced beam, impact and muzzle emission plus muzzle dynamic light. Multiplies the existing muzzle-light setting at the muzzle only. 0 off, 1 original, up to 8; live.");
	...
	r_pathTracingSunScale = ri.Cvar_Get("r_pathTracingSunScale", "1", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_pathTracingSunScale, 0.0f, 16.0f, qfalse);
	...
	r_pathTracingAutoExposure = ri.Cvar_Get("r_pathTracingAutoExposure", "0", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_pathTracingAutoExposure, 0, 1, qtrue);
	r_pathTracingAdaptiveSpeed = ri.Cvar_Get("r_pathTracingAdaptiveSpeed", "0.25", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_pathTracingAdaptiveSpeed, 0.01f, 10.0f, qfalse);
	r_pathTracingAdaptiveTarget = ri.Cvar_Get("r_pathTracingAdaptiveTarget", "0.18", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_pathTracingAdaptiveTarget, 0.0001f, 20.0f, qfalse);
	r_pathTracingAdaptiveMin = ri.Cvar_Get("r_pathTracingAdaptiveMin", "0.05", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_pathTracingAdaptiveMin, 0.0f, 64.0f, qfalse);
	r_pathTracingAdaptiveMax = ri.Cvar_Get("r_pathTracingAdaptiveMax", "16", CVAR_ARCHIVE);
	ri.Cvar_CheckRange(r_pathTracingAdaptiveMax, 0.0f, 4096.0f, qfalse);
	...
	r_pathTracingDlightReservoir = ri.Cvar_Get("r_pathTracingDlightReservoir", "0", CVAR_CHEAT);
	ri.Cvar_CheckRange(r_pathTracingDlightReservoir, 0, 1, qtrue);
