...
cvar_t* r_pathTracingRRDebug;
cvar_t* r_pathTracingRRDebugActive;
...
void R_Register( void ) 
{
	...
	r_pathTracingRRDebug = ri.Cvar_Get("r_pathTracingRRDebug", "0", CVAR_TEMP);
	ri.Cvar_CheckRange(r_pathTracingRRDebug, 0, 1, qtrue);
	ri.Cvar_SetDescription(r_pathTracingRRDebug, "Show only this frame's PT Ray Reconstruction result, with exposure/tone conversion but no sharpen, bloom, NV, post effects, HUD or FG. Live, not saved; console/menus temporarily suspend it. Requires PT mode 2 and active RR.");
	r_pathTracingRRDebugActive = ri.Cvar_Get("r_pathTracingRRDebugActive", "0", CVAR_ROM);
