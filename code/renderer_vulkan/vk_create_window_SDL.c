...
#include "vk_streamline.h"
    ...
    /* The bundled SDL headers are platform-neutral; expose the Win32 WM arm. */
...
void vk_getInstanceProcAddrImpl(void)
{
    ...
    if (vk_sl_is_initialized()) {
        qvkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)
            vk_sl_get_instance_proc_addr(VK_NULL_HANDLE, "vkGetInstanceProcAddr");
    } else {
    ...
    ri.Printf(PRINT_ALL, " Get instance proc address. (%s)\n",
        vk_sl_is_initialized() ? "using NVIDIA Streamline" : "using SDL2");
}
...
#if defined(_WIN32) && defined(USE_NVIDIA_STREAMLINE)
    if (vk_sl_is_initialized())
    {
        ...
        if (SDL_GetWindowWMInfo(window_sdl, &window_info))
        {
            ...
            if (create_surface)
            {
                ...
                /* SDL supplies the native instance and HWND as a matched pair.
			 * Do not derive the instance from SDL_GetWindowWMInfo's address:
			 * SDL is a DLL in this build, while DLSS-G needs the application
			 * instance associated with the game window. */
			desc.hinstance = window_info.info.win.hinstance;
                desc.hwnd = window_info.info.win.window;
                if (create_surface(vk.instance, &desc, NULL, &vk.surface) == VK_SUCCESS)
                    return;
            }
        }
        ri.Error(ERR_FATAL, "Could not create the Streamline-routed Win32 Vulkan surface");
    }
#endif
