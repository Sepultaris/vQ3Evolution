        ...
        VkViewport* const vp, VkRect2D* const pRect)
{
	...
	else
	{
		...
		if (vk_temporal_scene_pass_active()) {
			const float scale_x = (float)vk_temporal_render_width() / (float)width;
			const float scale_y = (float)vk_temporal_render_height() / (float)height;
			X = (int)((float)X * scale_x);
			Y = (int)((float)Y * scale_y);
			W = (int)((float)W * scale_x);
			H = (int)((float)H * scale_y);
			width = (int)vk_temporal_render_width();
			height = (int)vk_temporal_render_height();
		}
...
VkRect2D get_scissor_rect(void)
{
	...
	else
	{
		...
		if (vk_temporal_scene_pass_active()) {
			const float scale_x = (float)vk_temporal_render_width() / (float)width;
			const float scale_y = (float)vk_temporal_render_height() / (float)height;
			r.offset.x = (int32_t)((float)backEnd.viewParms.viewportX * scale_x);
			r.offset.y = (int32_t)((float)backEnd.viewParms.viewportY * scale_y);
			r.extent.width = (uint32_t)((float)backEnd.viewParms.viewportWidth * scale_x);
			r.extent.height = (uint32_t)((float)backEnd.viewParms.viewportHeight * scale_y);
			width = (int)vk_temporal_render_width();
			height = (int)vk_temporal_render_height();
		} else {
