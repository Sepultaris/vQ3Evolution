...
static void vk_selectGraphicsQueueFamily(void)
{
    ...
    /*
     * Select graphics before surface creation. Streamline installs its Win32
     * surface hook while the logical device is created, so the real surface
     * must be created afterwards for Frame Generation to associate its HWND.
     */
...
static void vk_createLogicalDevice(void)
{
    ...
    for (uint32_t i = 0; i < requested_extension_count; ++i)
    {
        const char *requested = vk_sl_device_extension(i);
        VkBool32 found = VK_FALSE;
        for (j = 0; j < nDevExts; ++j)
        {
            if (!strcmp(requested, pDeviceExt[j].extensionName))
            {
                found = VK_TRUE;
                break;
            }
        ...
        if (!found)
        {
            ri.Printf(PRINT_WARNING, "Streamline requested unavailable device extension %s\n", requested);
            continue;
        }
...
void vk_getProcAddress(void)
{
    ...
    vk_selectGraphicsQueueFamily();

    vk_createLogicalDevice();

    vk_sl_check_feature_support(vk.instance, vk.physical_device, vk.device);

    // Get device level functions.
    vk_loadDeviceFunctions();

    /* Create through Streamline after its device-time hooks are installed. */
	vk_createSurfaceImpl();

    VkBool32 presentation_supported = VK_FALSE;
    VK_CHECK(qvkGetPhysicalDeviceSurfaceSupportKHR(vk.physical_device,
        vk.queue_family_index, vk.surface, &presentation_supported));
    if (!presentation_supported)
        ri.Error(ERR_FATAL, "Vulkan: selected graphics queue cannot present to this window");
